...
public class Grain {
	...
	public Grain(byte[] clave, byte[] semilla, byte[] textoPlano) throws Exception {
		...
		this.planText = textoPlano;
		this.keystream = toByteArray(keystreamGenerator.generarKeystream(this.planText.length - 54));

		if ((this.longitud = this.planText.length) - 54 != this.keystream.length) {
			throw new Exception();
		}
	}
	...
	public byte[] xor() {
		...
		byte[] header = getHeader(this.planText);
		byte[] body = getBody(this.planText);
	...
	private byte[] toByteArray(short[] s) {
		...
		for (int i = 0; i < (s.length / 8); i++) {
			b[i] = (byte) (s[i * 8] * 128 + s[i * 8 + 1] * 64 + s[i * 8 + 2] * 32 + s[i * 8 + 3] * 16 + s[i * 8 + 4] * 8
					+ s[i * 8 + 5] * 4 + s[i * 8 + 6] * 2 + s[i * 8 + 7]);
		}
		return b;
	}
